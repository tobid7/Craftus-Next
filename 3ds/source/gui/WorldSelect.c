...
void WorldSelect_Render() {
	...
	if (menustate == MenuState_SelectWorld) {
		...
		clicked_play = Gui_Button(selectedWorld != -1, 0.5f, "Play");
		clicked_options = Gui_Button(true, 0.5f, "Options");
		Gui_EndRow();
		Gui_BeginRowCenter(Gui_RelativeWidth(0.95f), 2);
		clicked_new_world = Gui_Button(true, 0.5f, "New World");
		clicked_delete_world = Gui_Button(selectedWorld != -1, 0.5f, "Delete World");
		Gui_EndRow();
		...
		canceled_deletion = Gui_Button(true, 0.4f, "No");
		Gui_Space(0.2f);
		confirmed_deletion = Gui_Button(true, 0.4f, "Yes");
		Gui_EndRow();
		...
		Gui_Space(0.1f);
		if (Gui_Button(true, 0.45f, "%s", worldGenTypesStr[worldGenType])) {
			worldGenType++;
		...
		Gui_Space(0.1f);
		if (Gui_Button(true, 0.45f, "%f", fovscale)) {
			fovscale+=0.1f;
		...
		Gui_Space(0.1f);
		if (Gui_Button(true, 0.45f, "%s", diffstr[diffz])) {
			diffz++;
		...
		Gui_BeginRowCenter(Gui_RelativeWidth(0.9f), 3);
		canceled_world_options = Gui_Button(true, 0.45f, "Cancel");
		Gui_Space(0.1f);
		confirmed_world_options = Gui_Button(true, 0.45f, "Continue");
	}
}
