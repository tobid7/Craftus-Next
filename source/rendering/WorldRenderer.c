...
static void renderWorld() {
	...
	//DebugUI_Text("FC: %d", world->freeChunks.length);
}
...
void WorldRenderer_Render(float iod) {
	...
	C3D_TexBind(0, Block_GetTextureMap()); // Rebind Blocks Tex for Hand
    if (rhandr) {
		C3D_Mtx p;
		Mtx_PerspStereoTilt(&p, C3D_AngleFromDegrees(60.f), ((400.f) / (240.f)), camera.near, camera.far, iod, 1.f, false);
		Hand_Draw(projectionUniform, &p, player->quickSelectBar[player->quickSelectBarSlot], player);
	}
