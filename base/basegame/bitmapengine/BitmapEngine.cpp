...
bool NameIsEndingWith(const std::string &name,
  ...
  for (int i = 0; i < (int)extensions.size(); i++) {
    ...
    if (__stricmp(name.c_str() + name.size() - ext.size(), ext.c_str()) == 0)
      return true;
  }
...
void Base::BitmapPrinter::DrawPixel(int x, int y, uint8_t b, uint8_t g,
                                    uint8_t r, uint8_t a) {
  unsigned error =
      bitmap.set_pixel(x, (bitmap.bmp_info_header.height - 1) - y, b, g, r, a);
  if (error) {
  }
}
void Base::BitmapPrinter::DrawRect(int x, int y, int w, int h, uint8_t line_w,
...
void Base::BitmapPrinter::DrawRectFilled(int x, int y, int w, int h, uint8_t b,
...
void Base::BitmapPrinter::ClearBlank() { bitmap = blank; }
...
void Base::BitmapPrinter::Benchmark() {
  ...
  if (benchmark) {
    ...
    // this->DrawDebugText(20, 20, 0, Base::Color::Hex("#ffffff"), "Fps: " +
    // std::to_string(fps));
    /*this->DrawDebugText(0, 0, 0.5f, Base::Color::Hex("#ff0000"), "Time: " +
    std::to_string(timer)); this->DrawDebugText(0, 10, 0.5f,
    Base::Color::Hex("#ff0000"), "Fps: " + std::to_string(fps));
    this->DrawDebugText(0, 20, 0.5f, Base::Color::Hex("#ff0000"), "dt: " +
    std::to_string(dtt)); this->DrawDebugText(0, 30, 0.5f,
    Base::Color::Hex("#ff0000"), "MaxEncodeTime: " + std::to_string(mdtt2*1000)
    + "ms/f"); this->DrawDebugText(0, 40, 0.5f, Base::Color::Hex("#ff0000"),
    "MaxDecodeTime: " + std::to_string(mdtt3*1000) + "ms");*/
