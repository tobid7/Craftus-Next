...
namespace Base
{
    ...
    void Font::LoadTTF(std::string path)
    {
        ...
        for (unsigned char c = 0; c < 128; c++)
        {
            ...
            texture->LoadPixels(face->glyph->bitmap.buffer, face->glyph->bitmap.width, face->glyph->bitmap.rows, 1);
            // now store character for later use
            FChar character = {
                texture,
                bvec2i(face->glyph->bitmap.width, face->glyph->bitmap.rows),
    ...
    {
        textshader->use();
        glm::mat4 projection = glm::ortho(0.0f, (float)raster_box.x, 0.0f, (float)raster_box.y);
        ...
        for (c = text.begin(); c != text.end(); c++)
        {
            ...
            tempx += (ch.advance >> 6) * scale; // bitshift by 6 to get value in pixels (2^6 = 64)
        }
    }
}