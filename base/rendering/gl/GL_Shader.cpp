...
namespace Base {
void GL_Shader::LoadFile(const char *vertexPath, const char *fragmentPath,
...
void GL_Shader::LoadCode(const char *vertexShaderCode,
  ...
  std::string fragmentCode;
  std::string geometryCode;
  std::istringstream vShaderFile(vertexShaderCode);
  std::istringstream fShaderFile(fragmentShaderCode);
  std::istringstream gShaderFile;
  ...
  try {
    ...
    if (geometryShaderCode != nullptr) {
      gShaderFile = std::istringstream(geometryShaderCode);
      std::stringstream gShaderStream;
      gShaderStream << gShaderFile.rdbuf();
      geometryCode = gShaderStream.str().c_str();
    }
  } catch (std::ifstream::failure &e) {
    std::cout << "ERROR::SHADER::FILE_NOT_SUCCESSFULLY_READ: " << e.what()
              << std::endl;
...
void GL_Shader::setVec3(const std::string &name, float x, float y,
                        float z) const {
...
void GL_Shader::setVec4(const std::string &name, float x, float y, float z,
                        float w) {
...
void GL_Shader::checkCompileErrors(unsigned shader, std::string type) {
  GLint success;
  GLchar infoLog[1024];
  if (type != "PROGRAM") {
